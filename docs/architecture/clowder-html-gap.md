...
|------|------------|----------------|----------|
| Hub 指挥中心 | 有 Capability、Skills、Quota、配置等多 tab 指挥面板 | 原来只有 Agent 配置抽屉 | 新增轻量 `myteam Hub`，集中展示状态和任务；差距对比只保留在文档 |
...
| Skills 框架 | 技能按需加载，并展示每个技能挂载到哪些 agent | 已新增 `.myteam/skills.yaml`、Hub Skills 看板和按需 prompt 注入 | 后续升级成更细的渐进式文件加载 |
...
3. Hub 包含六个 tab：
   ...
   - Agent：启动级检测结果和错误原因。
   - Skills：技能、触发条件、当前命中 skill 和 prompt preview。
...
2. 做本地 RAG：从 HANDOVER / ISSUES / docs / lessons / tasks 检索上下文再注入 prompt。
