...
## 课程 23：结构化协议不能直接进入用户正文

标签：`plan`, `stream`, `json`, `ux`, `lesson`

计划 Agent 输出的 JSON 是机器间契约，即使内容正确，直接流进聊天气泡也会形成难以阅读的文本墙，还会把真正的任务卡推到视口之外。生成阶段应只展示可理解的进度，解析成功后再用任务对象渲染标题、负责人、原因、步骤、取舍、待确认项和验收标准；原始协议仅在失败诊断时按需提供。

复用提醒：不要“把 JSON 做得更好看”，而要让 UI 消费解析后的领域对象。

## 课程 24：临时错误、文件集合和会话状态不能都写进聊天正文

标签：`ux`, `artifact`, `state-machine`, `lesson`

“文件不存在”属于瞬时交互反馈，应使用可去重 Toast 并在原控件上标记失效；Agent 交付文件属于会话级集合，应进入可预览的文件面板；用户主动停止属于持久运行状态，应显示恢复入口。三者都追加成系统气泡，会污染历史、重复报错，也让用户找不到真正的交付物和下一步动作。

复用提醒：先判断信息寿命和作用域，再选择 Toast、面板、状态卡或历史消息。

## 课程 25：中断恢复必须等待旧执行收口

标签：`agent`, `abort`, `resume`, `race`, `lesson`

如果点击停止后立刻把会话标记为可继续，新请求可能在旧子进程的 close handler 之前启动，造成旧回复写到新用户消息之后，甚至由旧 handler 错误完成新 run。应使用 `running → interrupting → interrupted → running`，停止接口等待子进程 close；超时则保持 `interrupting` 并由前端短轮询，只有 `interrupted` 才展示继续按钮。

复用提醒：恢复能力的前提不是保存一个按钮，而是让旧执行先拥有确定的终态。
